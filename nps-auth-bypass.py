...
def admin_cores(url, cmd):
    ...
    r = requests.get(url=core_selector_url, headers=comheaders, verify=False)
...
def show_config(url,jas502n_Core_Name):
    ...
    r1 = requests.get(config_url, headers=comheaders, verify=False)
...
def get_config_name(url,jas502n_Core_Name):
    ...
    r2 = requests.get(get_config_url, headers=comheaders, verify=False)
...
def URLDataSource_Poc(url,jas502n_Core_Name,cmd):
    ...
    r3 = requests.post(url = debug_model_url, data=payload,headers=headers, verify=False)
